...
(() => {
  ...
  const game = (language) => {
    ...
    language = prompt(
      'If you want play in English enter ENG or EN',
    ).toUpperCase();
    const lang =
    ...
    return function start() {
      ...
      let computerChoice = '';
      if (userChoice === null) {
        const end = confirm(questions[1]);
        alert(questions[2] + result.player + questions[3] + result.computer);
        return end;
      }
      ...
      if (computerChoice === userChoice) {
        alert(questions[4] + userChoice + questions[7] + computerChoice);
      } else if (
        (computerChoice === lang[0] && userChoice === lang[2]) ||
        (computerChoice === lang[2] && userChoice === lang[1]) ||
        (computerChoice === lang[1] && userChoice === lang[0])
      ) {
        alert(questions[5] + userChoice + questions[7] + computerChoice);
        result.player += 1;
      } else {
        alert(questions[6] + userChoice + questions[7] + computerChoice);
        result.computer += 1;
      }
      start();
    };
